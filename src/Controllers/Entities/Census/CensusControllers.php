<?php

namespace App\Controllers\Entities\Census;

use App\Models\Entities\Census\CensusModels;

/**
 * Class CensusControllers
 * 
 * This is the controller class for handling CRUD operations of Census entities
 * 
 * @package App\Controllers\Entities\Census
 */

class CensusControllers
{
    /**
     * @var CensusModels
     */
    private CensusModels $_model;

    /**
     * @param CensusModels $model the CensusModels instance 
     */
    public function __construct(CensusModels $model)
    {
        $this->_model = $model;
    }

    /**
     * Save a new census.
     *
     * @return array Result of the operation.
     */
    public function save(): array
    {
        if (!isset($_POST['id'])) {
            return [
                'status' => 'fail',
                'message' => 'It is necessary to send the id and description to be added'
            ];
        };

        return $this->_model->save($_POST['id'], $_POST['description']);
    }

    /**
     * Search census according to given conditions.
     *
     * @return array Result of the search operation.
     */
    public function find(): array
    {
        return $this->_model->find($_POST);
    }

    /**
     * Updates census based on given values ​​and conditions.
     *
     * @return array Result of the update operation.
     */
    public function set(): array
    {
        if (!isset($_POST['values']) || !isset($_POST['conditions'])) {
            return [
                'status' => 'fail',
                'message' => ['It is necessary that you have the columns to modify and the conditions of the change']
            ];
        }
        return $this->_model->set($_POST['values'], $_POST['conditions']);
    }

    /**
     * Eliminates census according to given conditions.
     *
     * @return array Result of the delete operation.
     */
    public function delete(): array
    {
        if (!isset($_POST['conditions'])) {
            return [
                'status' => 'fail',
                'message' => ['It is necessary that you have the conditions of the delete']
            ];
        }
        return $this->_model->delete($_POST['conditions']);
    }
}
