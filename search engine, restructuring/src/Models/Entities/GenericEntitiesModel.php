...
class GenericEntitiesModel implements IGenericEntities
{
    ...
    private ValidationGenericEntitiesModel $validate;
    private string $_databaseType;
    public function __construct(string $databaseType)
    {
        $this->db = new DBPrepareQuery($databaseType);
        $this->_databaseType = $databaseType;
        $this->validate = new ValidationGenericEntitiesModel();
    }

    public function save(string $table, array $values): array | string
    {
        $isValid = $this->validate->validateNameTable($table);
        if ($isValid['status'] !== 'ok') return $isValid;

        $isValid = $this->validate->validateArray($values);
        if ($isValid['status'] !== 'ok') return $isValid;

        if (!isset($values['id'])) return $this->templateMessageResponse('fail', ['When saving, an ID is always required to be sent.']);
        ...
        if (count($result['response']) > 0) return $this->templateMessageResponse('fail', ['Existing record']);

        $sql = "INSERT INTO $table ($columns) VALUES ($placeholders)";

        // Ejecutar la consulta y retornar el resultado
        // return $this->db->searchData($sql, $values);

        $result = $this->db->searchData($sql, $values);
        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse('ok', ['Saved information successfully.'], $result);
    }
    ...
    public function callProcedure(string $name, array $values): array
    {
        ...
        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse('ok', ['Store Procedure Execute successfully.'], $result);
    }
    ...
    public function get(string $table, array $values = null): array | string
    {
        ...
        $isValid = isset($values) ? $this->validate->validateArray($values) : ['status' => 'ok'];
        if ($isValid['status'] !== 'ok') return $isValid;
        ...
        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse('ok', ['Search completed successfully.'], $result);
    }
    ...
    public function find(string $table, array $mixed = null): array | string
    {
        ...
        $isValid = isset($mixed['select']) ? $this->validate->validateArray($mixed['select']) : ['status' => 'ok'];
        if ($isValid['status'] !== 'ok') return $isValid;

        $isValid = isset($mixed['conditions']) ? $this->validate->validateArray($mixed['conditions']) : ['status' => 'ok'];
        if ($isValid['status'] !== 'ok') return $isValid;
        ...
        $result = $this->db->searchData($sql, $mixed['conditions']);

        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse(
                'ok',
                count($result) > 0 ?
    ...
    {
        $isValidTable = $this->validate->validateNameTable($table);
        if ($isValidTable['status'] !== 'ok') return $isValidTable;

        $isValidArray = $this->validate->validateArray($values);
        if ($isValidArray['status'] !== 'ok') return $isValidArray;

        $isValidArray = $this->validate->validateArray($conditions);
        if ($isValidArray['status'] !== 'ok') return $isValidArray;
        ...
        $result = $this->db->searchData($sql, $newArray);
        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse('ok', ['Changes made successfully.'], $result);
    }
    ...
    public function delete(string $table, array $conditions): array | string
    {
        ...
        $isValid = $this->validate->validateArray($conditions);
        if ($isValid['status'] !== 'ok') return $isValid;
        ...
        $result = $this->db->searchData($sql, $conditions);
        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse('ok', ['Records successfully deleted.'], $result);
    }
    ...
    public function lastId(string $table): array | string
    {
        ...
        $result = $this->db->searchData($sql);
        return is_string($result)
            ? $this->templateMessageResponse('fail', [$result])
            : $this->templateMessageResponse('ok', ['Last ID searched successfully.'], $result);
    }

    private function templateMessageResponse(string $status, array $message, array $response = null): array
    {
        return [
            'status' => $status,
            'message' => $message,
